...
with tab3:
    ...
    with chart_col:
        ...
        if not rehab.empty:
            ...
            # Show labels for all slices >= 2.5%
            ...
            fig.update_layout(**CHART_BG, height=600, showlegend=False,
                uniformtext=dict(minsize=8, mode="hide"),
                margin=dict(l=140, r=170, t=30, b=90))
            render_chart(fig, height=650)
    ...
    if not props.empty:
        ...
        # Column order: All-In | Cost Category | Specific Cost | Total Cost (rightmost)
        tbl = tbl[[
            "Property Address", "_item_amount",
            "Property Walker", "Sq Ft", "Beds", "Baths", "Hold",
            "CoC %", "Net Profit", "ARV", "Buy Price",
            "All-In", "Cost Category", "Specific Cost", "Total Cost",
        ]]
        ...
        gb2.configure_column("Cost Category", rowGroup=True, cellRenderer=r_cat, width=150, hide=True,
        ...
        gb2.configure_column("Baths",    aggFunc="first", type=["numericColumn"], cellRenderer=r_num,    width=82)
        gb2.configure_column("Hold",     aggFunc="first", type=["numericColumn"], cellRenderer=r_num,    width=78)
        gb2.configure_column("CoC %",    aggFunc="first", type=["numericColumn"], cellRenderer=r_pct,    width=90)
        gb2.configure_column("Net Profit", aggFunc="first", type=["numericColumn"], cellRenderer=r_dollar, width=105)
